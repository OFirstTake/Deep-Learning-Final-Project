...
# Define input structure: Require the user to input Company and Date
...
def predict_stock(data: StockQuery):
    try:
        # Check if the requested company is in the supported list
        company = data.company.upper()
        if company not in VN_technology:
            raise HTTPException(status_code=400, detail=f"Stock company {company} is not in the supported list!")

        # Automatically find the CSV file for the company in the specified directory
        matching_files = glob.glob(f"data-vn-20230228\\stock-historical-data\\{company}-*.csv")
        
        if not matching_files:
            raise HTTPException(status_code=404, detail=f"CSV file for company {company} not found. Please check the data directory!")
        ...
        if target_date not in df['Trading Date'].values:
            raise HTTPException(status_code=404, detail=f"No trading data available for date {data.date} of company {company}!")
        ...
        return {
            ...
            "history_dates": window_df['Trading Date'].dt.strftime('%Y-%m-%d').tolist(),
            "history_prices": window_df['Open'].tolist()
        }
